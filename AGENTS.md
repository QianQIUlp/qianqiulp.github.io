...
All visual changes follow the modern, person-first room direction documented in `docs/uiux/homepage-art-direction.md`, `docs/uiux/person-first-intent.md`, and the active implementation contract at `docs/uiux/ink-and-light-study.md` (the filename is retained for link stability). Preserve the real room, guitar, three distinct project effects units, warm light, restrained red accent, progressive discovery, Light/Dark themes, and comfortable long-form reading. Do not reintroduce calligraphic/QiuBrush UI, xuan-paper texture or global noise, petals, ink-fluff particles, or seals. Copy is frozen to `f6ed8fd` except for four explicit changes: remove 「展开卷宗」 and its English counterpart, change Chinese blog counts from 「卷 / 题」 to 「篇 / 标签」, remove the generated 「题跋」 label, and remove the visible 「授权协议 · 文末记」 block. Do not rewrite any other visible or assistive copy as part of visual cleanup; visible theme labels remain 「昼 / 夜」 and `Day / Night`. Read the three documents before touching styles or components, follow `docs/qa/visual-checklist.md`, and update the contract plus `.agents/skills/qiu-site-frontend-design/` when changing a governed system. Scene asset regeneration prompts live in `docs/asset-prompts/`.
