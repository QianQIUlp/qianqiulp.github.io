...
好消息是，个人 VPS 不需要先把自己变成安全专家。先把最危险的默认入口收住，再补上自动更新和登录防护，已经能避开大量自动化扫描与弱配置风险。这里写的是我给个人服务器用的做法；多用户生产环境或有合规要求的系统，不能直接照搬。
...
<aside>
**常见坑**：`adduser` 是交互式的，中途按到 `Ctrl+Z` 会挂起、密码不被设上。出问题就：
...
```
<aside>
**权限必须正确**，否则 sshd 拒绝使用密钥还**不告诉你为什么**：
...
云服务器的优势：**有控制台救援通道**。
阿里云：`实例 → 远程连接 → Workbench / VNC` —— 这条通道**绕过 SSH 独立工作**，fail2ban 封不到。
进去后：
```
...
我很容易把服务器加固做成一场“找最优解”的考试。但个人服务器真正需要的不是一个漂亮分数，而是默认入口已经收住、每一步都验证过、出错时还有路能回去。

基础措施带来的收益最大。再往后当然还可以加更多审计、监控和隔离，但那应该由真实需求推动，而不是因为清单永远列不完。做到当前用途够用，我就先停下来使用它；以后遇到新的服务和风险，再补对应的一层。
...
我给新服务器做完这一轮之后，不会觉得它从此“安全了”。我只知道几件具体的事：平时不用 root，密码登录已经关掉，入口先经过安全组，补丁和失败登录有人持续照看；更重要的是，改 SSH 时我始终留着旧会话和云控制台这条退路。

这就够我开始用这台机器了。

以后再装 Redis、数据库或别的网络服务，我会重新问一遍：它真的需要监听公网吗？如果不需要，就只绑在 `127.0.0.1`。安全对我来说不是一次配完的满分答案，而是每加一个东西，都没有顺手多开一扇门。
